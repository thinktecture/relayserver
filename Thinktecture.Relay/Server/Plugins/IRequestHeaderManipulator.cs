...
{
	/// <summary>
	/// Interface for plugins that are capable of modifying the header of an incoming request before it gets relayed to its destination.
	/// </summary>
	public interface IRequestHeaderManipulator
	{
		/// <summary>
		/// This method can replace the http headers of a request if it returns a new header collection to replace the old.
		/// </summary>
		...
		/// <returns>Return the new header collection that will replace, not extend, the headers on the provided <paramref name="request"/>.
