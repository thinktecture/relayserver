...
namespace Thinktecture.Relay.Server.Protocols.RabbitMq
{
	...
	public class ModelFactory
	{
		...
		public ModelFactory(ILogger<ModelFactory> logger, IConnection connection)
		{
			...
			{
				autorecoveringConnection.RecoverySucceeded += (sender, args) => _logger.LogInformation("Connection successful recovered");
			}

			connection.ConnectionShutdown += (sender, args) => _logger.LogDebug("Connection closed ({ShutdownReason}", args.ReplyText);
		}
		...
		public IModel Create(string context)
		{
			...
			model.CallbackException += (sender, args) => _logger.LogError(args.Exception,
				"An error occured in a model callback for {ModelContext} with channel {ModelChannel}", context, model.ChannelNumber);
