...
1. `RelayMiddleware`
   ...
   - If the request has a body and it is larger than the maximum size the server to server [transport](#transports) allows, it will be
...
1. `TenantHandler`  
   The tenant handler is the listener on the RabbitMQ server-to-server transport that is instanciated for each connector connection and
...
- The client request handler first checks, if the requested target is available. If this is not the case, it answers the request with a 404
...
To make relaying possible without configuring port forwardings in a router or opening additional ports on firewalls, the Connector is meant
...
backchannel which will be used for handshake and server-side Connector configuration as well as heartbeating and acknowledgment.
...
these cases the body is seperated from the actual request or response and transferred via normal HTTP GET (downloading of the request body
...
The default (`AcknowledgeMode.ConnectorReceived`), is sent by the Connector via the Connector transport backchannel when the request was
...
acknowledging the message through the Connector transport backchannel.
...
the queue anexplicit acknowledge HTTP POST request is sent to the RelayServer by some custom code. The url with all arguments required for
