...
namespace Thinktecture.Relay.Server.Config
{
	public class LocalAppDataPersistedSettings : IPersistedSettings
	{
		...
		private string DetermineSettingsFileName()
		{
			...
			_logger?.Debug("Using settings file '{settings-file-name}'", fileName);
		...
		private void Load()
		{
			if (File.Exists(_settingsFileName))
			{
				...
				_logger?.Debug("Loaded setting with origin id {origin-id}", OriginId);
			}
			else
			{
		...
		private void Store()
		{
			...
			_logger?.Debug("Stored setting with origin id {origin-id}", OriginId);
		}
